...
async function run() {
  ...
  // register/login to obtain token then list orders
  const acctBase = `http://localhost:${process.env.PORT||3000}/api/v1`;
  const email = `otest+${Date.now()}@example.com`;
  const password = 'Password123!';
  const reg = await request({ url: `${acctBase}/accounts/register`, method: 'POST', body: { email, password, name: 'OTest' } });
  ...
  const list = await request({ url: `${base}/orders`, method: 'GET', headers: { Authorization: `Bearer ${token}` } });
  if (list.status >= 400) throw new Error('listOrders failed: ' + JSON.stringify(list.body));
  console.log('orders_success: ok');
}

if (require.main === module) {
  run().catch((e) => {
    console.error('orders_success: failed', e.message || e);
    process.exit(1);
  });
}
