...
const { server } = require("../prisma");
const { createNotificationHandlers } = require("../handlers/notificationHandler");
const { createLogHandlers } = require("../handlers/logHandler");

function serverResourcesRoutes() {
  const router = express.Router();
  const notifications = createNotificationHandlers(server);
  const logs = createLogHandlers(server);
...
module.exports = serverResourcesRoutes;
